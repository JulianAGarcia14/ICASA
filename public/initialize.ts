...
class net {
    ...
    nodeIds: number[];
    clusterSum: number;
    nodes_array: Array<Object>;
    edges_array: Array<Object>;
    nodes: vis.DataSet<any>;
    edges: vis.DataSet<any>;
    degreeDist: Array<Object>;
    sumOfDegrees: number;
 ...
 */
function erdos(pr: string, vn: string, network: net, globeContainer: string, nodeContainer: string, graphContainer: string) {
 ...
 */
function neighborConnect(neighborhood, used, idNum, network: net) {
 ...
 */
function connect(idNum, isLooped, network: net) {
 ...
 */
function createAdvanced(vn: string, network: net, isLooped: boolean, globeContainer: string, nodeContainer: string, graphContainer: string) {
    let vertNum = parseInt(vn);
    let idLen = network.nodes.length;
    //var idLen = Object.keys(nodes).length / 2;
    let idNum;
 ...
 * @param  {string} graphContainer container to draw degree distribution graph
 */
function writedata(network: net, globeContainer: string, nodeContainer: string, graphContainer: string) {
    let idLen = network.nodes.length;
    let arr = new Array(5);
    let localCluster;
    arr.fill(0);
 ...
 */
function clusteringCount(currNode: number, network: net) {
    let totalE = 0;
    let atMost;
 ...
 * @param  {string} graphContainer container to draw degree distribution graph
 */
function drawDistribution(network: net, graphContainer: string) {
    // @ts-ignore
